...
public:
	...
	static DbRecover* check(
		const char* file, bool (*progress)(int) = null_progress);
	virtual DbrStatus check_indexes(bool (*progress)(int) = null_progress) = 0;
	virtual DbrStatus status() = 0;
	virtual const char* last_good_commit() = 0;
	virtual bool rebuild(
		bool (*progress)(int) = null_progress, bool log = true) = 0;
};
...
struct Commit {
	...
	Mmoffset32* deletes() {
		return reinterpret_cast<Mmoffset32*>(
			(char*) this + sizeof(Commit) + ncreates * sizeof(Mmoffset32));
	}
	size_t size() const {
