...
private:
	bool matches(char c) const {
		return ignoringCase ? cm->matches(tolower(c)) || cm->matches(toupper(c))
							: cm->matches(c);
	}
	CharMatcher* cm;
	bool ignoringCase;
...
void RxCompile::charClass() {
	...
	while (si < sn && src[si] != ']') {
		...
		if (matchRange()) {
			...
			elem = (from < to) ? CharMatcher::inRange(from, to)
							   : &CharMatcher::NONE;
		} else if (match("\\d"))
			elem = digit;
		else if (match("\\D"))
 ...
 * amatch matches a specific point in a string against a compiled regular
 * expression returns -1 if no match, else the position past the match
 */
class RxMatch {
public:
	...
	int amatch(
		int si, const Element** pat, IntList* alt_si, IntList* alt_pi) const;
...
bool rx_match_reverse(
	const char* s, int n, int i, const char* pat, Rxpart* psubs) {
	IntList alt_si;
	IntList alt_pi;
	RxMatch match(s, n, psubs);
...
static char trcase(char& tr, char c) {
	c = tr == 'E' ? c : (tr == 'L' || tr == 'l' ? tolower(c) : toupper(c));
	if (tr == 'u' || tr == 'l')
		tr = 'E';
	return c;
}
...
TEST(regex_match) {
	for (int i = 0; i < sizeof rxtests / sizeof(Rxtest); ++i) {
		...
		except_if(rxtests[i].result !=
				rx_match(string, strlen(string), 0, pat, nullptr),
			string << " =~ " << pattern << " should be "
				   << (rxtests[i].result ? "true" : "false"));
	}
}
...
TEST(regex_omatch) {
	...
	CharMatcher* cm =
		CharMatcher::anyOf("acd")->or_(CharMatcher::inRange('1', '9'));
