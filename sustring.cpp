...
Value SuString::call(Value self, Value member, short nargs, short nargnames,
...
Value SuString::Match(short nargs, short nargnames, short* argnames, int each) {
	...
	const char* t = ptr();
	bool hasmatch = prev
		? rx_match_reverse(t, size(), pos, rx_compile(pat), parts)
...
Value SuString::Replace(
	short nargs, short nargnames, short* argnames, int each) {
	if (nargs < 1 || 3 < nargs)
		except("usage: "
			   "string.Replace(pattern, replacement = '', count = false) "
			   "-> string");
	// replacement
...
Value SuString::MapN(short nargs, short nargnames, short* argnames, int each) {
	...
	int n = ARG(0).integer();
	Value block = ARG(1);
	Buffer dst; // don't allocate large buffer because MapN may be used just for
				// side effects
...
Value SuStringIter::call(Value self, Value member, short nargs, short nargnames,
	short* argnames, int each) {
...
TEST(sustring_replace) {
	...
	assert_eq(
		SuString("hello world").replace("l", "L", 0), "hello world"); // same
	assert_eq(SuString("hello world").replace(".", "<&>", 5),
		"<h><e><l><l><o> world");
