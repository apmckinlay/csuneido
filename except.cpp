...
Except::Except(gcstring x)
	: SuString(x.trim()), fp_fn_(tls().proc ? tls().proc->fp->fn : 0),
	  is_block_return(x == "block return") {
...
Except::Except(const Except& e, gcstring s)
	: SuString(s), fp_fn_(e.fp_fn_), calls_(e.calls_),
	  is_block_return(e.is_block_return) {
}
...
Value Except::call(Value self, Value member, short nargs, short nargnames,
