...
double Summarize::nrecords() {
	...
	return nr < 1 ? nr
				  : nil(by)
			? 1
			: by_contains_key() ? nr : nr / 2; // TODO review this estimate
}
...
double Summarize::seqCost(
	const Fields& index, const Fields& srcneeds, bool is_cursor, bool freeze) {
	if (freeze)
		strategy = SEQ;
	if (nil(by) || by_contains_key()) {
		...
		best_prefixed(sourceIndexes(index), by, srcneeds, is_cursor, best_index,
			best_cost);
...
double Summarize::mapCost(
	...
	double cost =
		1.5 * source->optimize1(none, srcneeds, none, is_cursor, freeze);
	if (freeze)
		strategy = MAP;
	return cost;
}
...
Fields Summarize::columns() {
	return wholeRecord ? set_union(cols, source->columns())
					   : set_union(by, cols);
}
...
void SeqStrategy::select(const Fields& index, Record from, Record to) {
	...
	if (prefix(q->via, index) || (from == keymin && to == keymax))
		...
		except_err("Summarize SeqStrategy by "
			<< q->via << " doesn't handle select(" << index << " from " << from
			<< " to " << to << ")");
}

//===================================================================

MapStrategy::MapStrategy(Summarize* summarize)
	: Strategy(summarize), first(true) {
}
