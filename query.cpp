...
// tempindex ?
double Query::optimize(const Fields& index, const Fields& needs,
	const Fields& firstneeds, bool is_cursor, bool freeze) {
	TRACE(QUERYOPT,
		"Query::optimize START "
			<< this << (freeze ? " FREEZE" : "") << endl
	...
	double no_index_cost =
		optimize1(none, tempindex_needs, firstneeds, is_cursor, false);
	...
	TRACE(QUERYOPT,
		"Query::optimize END "
			...
			<< "\twith TEMPINDEX cost " << cost2 << "(" << no_index_cost
			<< " + " << tempindex_cost << ") "
...
// cache
double Query::optimize1(const Fields& index, const Fields& needs,
	const Fields& firstneeds, bool is_cursor, bool freeze) {
	double cost;
...
void Query1::best_prefixed(Indexes idxs, const Fields& by, const Fields& needs,
	bool is_cursor, Fields& best_index, double& best_cost) {
	...
	for (; !nil(idxs); ++idxs) {
		...
		if (prefixed(ix, by, fixed)) {
			...
			double cost =
				source->optimize1(ix, needs, Fields(), is_cursor, false);
			if (cost < best_cost) {
				best_cost = cost;
				best_index = ix;
			}
...
970103	\"pencil\"	\"e\"	300\n"},
	...
	{"hist where item =~ 'e' where item =~ 'e'",
		"hist^(date,item,id) WHERE^(date,item,id) ((item =~ \"e\") and (item "
		"=~ \"e\"))", // 5
		"date	item	id	cost\n\
970102	\"mouse\"	\"c\"	200\n\
970103	\"pencil\"	\"e\"	300\n"},
...
970103	\"pencil\"	\"e\"	300\n"},
	...
	{"alias", "alias^(id)", "id	name2\n\
\"a\"	\"abc\"\n\
\"c\"	\"trical\"\n"},

	{"supplier", "supplier^(city)", "supplier	name	city\n\
...
\"i\"	\"intercon\"\n"},
	...
	{"trans project item", "trans^(item) PROJECT-SEQ^(item) (item)", "item\n\
\"disk\"\n\
\"eraser\"\n\
\"mouse\"\n"},

	{"trans project item,id,cost,date project item",
		"trans^(item) PROJECT-SEQ^(item) (item)", "item\n\
\"disk\"\n\
\"eraser\"\n\
\"mouse\"\n"},
...
970103	\"pencil\"\n"},
	...
	{"customer project city", "customer^(id) PROJECT-LOOKUP (city)", "city\n\
\"saskatoon\"\n\
\"calgary\"\n\
\"vancouver\"\n"},

	{"customer project id,city project city",
		"customer^(id) PROJECT-LOOKUP (city)", "city\n\
\"saskatoon\"\n\
\"calgary\"\n\
\"vancouver\"\n"},

	{"customer rename city to location",
		"customer^(id) RENAME city to location", "id	name	location\n\
\"a\"	\"axon\"	\"saskatoon\"\n\
\"c\"	\"calac\"	\"calgary\"\n\
\"e\"	\"emerald\"	\"vancouver\"\n\
\"i\"	\"intercon\"	\"saskatoon\"\n"},
	...
	{"customer rename city to location rename location to loc",
		"customer^(id) RENAME city to loc", "id	name	loc\n\
\"a\"	\"axon\"	\"saskatoon\"\n\
\"c\"	\"calac\"	\"calgary\"\n\
\"e\"	\"emerald\"	\"vancouver\"\n\
\"i\"	\"intercon\"	\"saskatoon\"\n"},
	...
	{"customer rename id to i rename name to n rename city to c",
		"customer^(id) RENAME id to i, name to n, city to c", "i	n	c\n\
\"a\"	\"axon\"	\"saskatoon\"\n\
\"c\"	\"calac\"	\"calgary\"\n\
\"e\"	\"emerald\"	\"vancouver\"\n\
\"i\"	\"intercon\"	\"saskatoon\"\n"},
	...
	{"inven rename item to part, qty to onhand",
		"inven^(item) RENAME item to part, qty to onhand", "part	onhand\n\
\"disk\"	5\n\
\"mouse\"	2\n\
\"pencil\"	7\n"},
...
\"pencil\"	7	\"i\"	\"intercon\"	\"saskatoon\"\n"},
	...
	{"hist union trans",
		"(hist^(date,item,id)) UNION-MERGE^(date,item,id) "
		"(trans^(date,item,id)) ",
		"date	item	id	cost\n\
960204	\"mouse\"	\"e\"	200\n\
970101	\"disk\"	\"a\"	100\n\
...
\"eraser\"	\"c\"	150	970201\n"},
	...
	{"hist intersect trans",
		"(hist^(date,item,id)) INTERSECT^(date,item,id) "
		"(trans^(date,item,id)) ",
		"date	item	id	cost\n\
970101	\"disk\"	\"a\"	100\n"},

	{"trans intersect hist",
		"(trans^(date,item,id)) INTERSECT^(date,item,id) "
		"(hist^(date,item,id)) ",
		"item	id	cost	date\n\
\"disk\"	\"a\"	100	970101\n"},
	// 29
	{"(trans union trans) intersect (hist union hist)",
		"((trans^(date,item,id)) UNION-MERGE^(date,item,id) "
		"(trans^(date,item,id)) ) INTERSECT^(date,item,id,cost) "
		"((hist^(date,item,id)) UNION-MERGE^(date,item,id) "
		"(hist^(date,item,id))  TEMPINDEXN(date,item,id,cost) unique) ",
		"item	id	cost	date\n\
\"disk\"	\"a\"	100	970101\n"},
...
\"mouse\"	2	\"c\"	200	970101\n"},
	...
	{"(trans union trans) join (inven union inven)",
		"((trans^(date,item,id)) UNION-MERGE^(date,item,id) "
		"(trans^(date,item,id)) ) JOIN n:n on (item) ((inven^(item)) "
...
\"c\"	\"trical\"	\"calac\"	\"calgary\"\n"},
	...
	{"(customer times inven) join trans",
		"(trans^(date,item,id)) JOIN n:1 on (id,item) ((customer^(id)) TIMES "
		"(inven^(item)) TEMPINDEXN(id,item) unique)",
...
\"mouse\"	\"c\"	200	970101	\"calac\"	\"calgary\"	2\n"},
	...
	{"trans join customer join inven",
		"((trans^(date,item,id)) JOIN n:1 on (id) (customer^(id))) JOIN n:1 on "
		"(item) (inven^(item))",
		"item	id	cost	date	name	city	qty\n\
\"mouse\"	\"e\"	200	960204	\"emerald\"	\"vancouver\"	2\n\
\"disk\"	\"a\"	100	970101	\"axon\"	\"saskatoon\"	5\n\
\"mouse\"	\"c\"	200	970101	\"calac\"	\"calgary\"	2\n"},
	...
	{"(trans where cost=100) union (trans where cost=200)",
		"(trans^(item) WHERE^(item)) UNION-DISJOINT (cost) (trans^(item) "
		"WHERE^(item)) ",
		"item	id	cost	date\n\
\"disk\"	\"a\"	100	970101\n\
\"mouse\"	\"e\"	200	960204\n\
\"mouse\"	\"c\"	200	970101\n"},
	...
	{"(trans join customer) union (hist join customer)",
		...
		"UNION-MERGE^(date,item,id) ((hist^(date,item,id)) JOIN n:1 on (id) "
		"(customer^(id))) ",
...
\"eraser\"	\"c\"	150	970201	\"calac\"	\"calgary\"\n"},
	...
	{"(trans join customer) intersect (hist join customer)",
		"((trans^(date,item,id)) JOIN n:1 on (id) (customer^(id))) "
		"INTERSECT^(date,item,id) ((hist^(date,item,id)) JOIN n:1 on (id) "
		"(customer^(id))) ",
		"item	id	cost	date	name	city\n\
\"disk\"	\"a\"	100	970101	\"axon\"	\"saskatoon\"\n"},
...
\"pencil\"	7\n"},
	...
	*/
	{"inven where qty + 1 in (3,8)",
		"inven^(item) WHERE^(item) ((qty + 1) in (3,8))", "item	qty\n\
\"mouse\"	2\n\
\"pencil\"	7\n"},
...
970103	\"pencil\"	\"e\"	300	\"pencile\"\n"},
	...
	{"inven extend x = -qty sort x",
		"inven^(item) EXTEND x = -qty TEMPINDEXN(x)", "item	qty	x\n\
\"pencil\"	7	-7\n\
\"disk\"	5	-5\n\
\"mouse\"	2	-2\n"},
...
\"mouse\"	400\n"},
	...
	{"trans summarize total cost",
		"trans^(item) SUMMARIZE-SEQ total_cost = total cost", "total_cost\n\
650\n"},

	{"inven rename qty to x where x > 4",
		"inven^(item) WHERE^(item) RENAME qty to x", "item	x\n\
\"disk\"	5\n\
\"pencil\"	7\n"},

	{"inven extend x = qty where x > 4",
		"inven^(item) WHERE^(item) EXTEND x = qty", "item	qty	x\n\
\"disk\"	5	5\n\
\"pencil\"	7	7\n"},
...
\"mouse\"	\"c\"	200	970101	1\n"},
	...
	// test sorting of IN
	{"customer where id in ('i', 'c', 'e', 'a') sort id",
		"customer^(id) WHERE^(id)", "id	name	city\n\
\"a\"	\"axon\"	\"saskatoon\"\n\
\"c\"	\"calac\"	\"calgary\"\n\
\"e\"	\"emerald\"	\"vancouver\"\n\
\"i\"	\"intercon\"	\"saskatoon\"\n"},
...
Querystruct querytests2[] = {
	...
	{"(((task join co)) join (cus where abbrev = 'a'))",
		"((co^(tnum)) JOIN 1:1 on (tnum) (task^(tnum))) JOIN n:1 on (cnum) "
		"(cus^(cnum) WHERE^(cnum))",
...
104	990103	1	\"a\"	\"axon\"\n"},
	...
	{"((task join (co where signed = 990103)) join (cus where abbrev = 'a'))",
		"((co^(tnum) WHERE^(tnum)) JOIN 1:1 on (tnum) (task^(tnum))) JOIN n:1 "
		"on (cnum) (cus^(cnum) WHERE^(cnum))",
		"tnum	signed	cnum	abbrev	name\n\
104	990103	1	\"a\"	\"axon\"\n"},
...
TEST(query) {
	...
	req(tran,
		"insert{date: 970101, item: \"disk\", id: \"a\", cost: 100} into "
		"hist2");
	req(tran,
		"insert{date: 970102, item: \"disk\", id: \"e\", cost: 200} into "
		"hist2");
	req(tran,
		"insert{date: 970103, item: \"pencil\", id: \"e\", cost: 300} into "
		"hist2");
	...
	req(tran,
		...
		"trans");
	req(tran,
		"insert{item: \"mouse\", id: \"c\", cost: 200, date: 970101} into "
		"trans");
	req(tran,
	...
	req(tran,
		...
		"into supplier");
	req(tran,
		"insert{supplier: \"ebs\", name: \"ebssail&sport\", city: "
		"\"saskatoon\"} into supplier");
	req(tran,
		"insert{supplier: \"taiga\", name: \"taigaworks\", city: "
		"\"vancouver\"} into supplier");
...
static void process2(int i) {
	...
	{ // read next
		...
		if (q->optimize(order, q->columns(), Fields(), false, true) >=
			IMPOSSIBLE)
			except("impossible");
		q = q->addindex();
...
#define TESTNUP(q) \
	except_if(query(q)->updateable(), #q "should NOT be updateable")
...
TEST(query_history) {
	...
	assert_eq(os.str(),
		gcstring("cnum: 1 abbrev: \"a\" name: \"axon\" _action: \"create\""));
	SuDate now;
...
TEST(query_prefixed) {
	...
	Fields index_abce =
		lisp(gcstring("a"), gcstring("b"), gcstring("c"), gcstring("e"));
	Fields by_acde =
		lisp(gcstring("a"), gcstring("c"), gcstring("d"), gcstring("e"));
