...
Value SuInstance::call(Value self, Value member, short nargs, short nargnames,
	...
	return const_cast<Value*>(&myclass)->call(
...
const char* SuInstance::toString() const {
	...
	if (c->hasMethod(ToString)) {
		KEEPSP
		if (Value x = c->call(
				const_cast<SuInstance*>(this), ToString, 0, 0, nullptr, -1))
