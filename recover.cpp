...
bool check_shutdown(Mmfile* mmf) {
	if (cmdlineoptions.ignore_check)
		return true;
	void* p = *--mmf->end();
	return (mmf->type(p) == MM_SESSION &&
		((Session*) p)->mode == Session::SHUTDOWN);
}
...
public:
	...
	Mmoffset operator[](Mmoffset from) {
		std::vector<TrEntry>::iterator it =
			std::lower_bound(map.begin(), map.end(), from);
		verify(it->from() == from);
		return it->to();
	}
...
static bool schema(
	Database& db, HashMap<TblNum, gcstring>& tblnames, Mmoffset o);
static void dbdump1(Ostream& log, Mmfile& mmf, Mmfile::iterator& iter,
	uint32_t& cksum, BitVector& deletes);
...
public:
	explicit DbRecoverImp(const char* f)
		: file(f), mmf(open_mmf(file)), deletes(mmf->size() / granularity),
		...
		if (iter.type() == MM_DATA) {
			...
			if (tblnum != TN_TABLES && tblnum != TN_INDEXES) {
				...
				if (!fieldsbug && !cmdlineoptions.unattended &&
					tbl_nextfield.find(tblnum) &&
					r.size() > tbl_nextfield[tblnum]) {
					alerts = true;
			...
			cksum = checksum(
				cksum, (char*) commit + sizeof(int), iter.size() - sizeof(int));
			...
			for (int i = 0; i < commit->ndeletes; ++i) {
				...
				// - sizeof (int) because these offsets are to data, which is
				// preceded by table number
			}
	...
	for (int i = 0; i < n; ++i)
		if (r1.getraw(i) != r2.getraw(i)) {
			if (r1.getval(i) != r2.getval(i))
				...
				os << " key len " << r2.getraw(i).size() << " iter->key len "
				   << r1.getraw(i).size();
				errlog(os.str());
			}
		}
		...
		for (Lisp<Idx> ix = tbl->idxs; !nil(ix); ++ix) {
			...
			for (; !iter.eof(); ++iter, ++n) {
				...
				if (iter->key != key && !sloweq(iter->key, key)) {
					OstreamStr os;
					os << "check indexes: " << tbl->name << " " << ix->columns
					   << " data does not match index";
					errlog(os.str());
					status_ = DBR_ERROR;
					break;
				}
			...
			if (n != tbl->nrecords) {
				OstreamStr os;
				os << "check indexes: " << tbl->name << " " << tbl->nrecords
				   << " != " << ix->columns << " " << n;
				errlog(os.str());
				status_ = DBR_ERROR;
				break;
...
static bool schema(
	...
	case TN_INDEXES:
		...
		if (!db.recover_index(r)) {
			...
			alert("recover index failed: " << tblnum << "=" << table
										   << (key ? " key(" : " index(")
										   << columns << ")");
			return false;
		}
...
void dbdump1(Ostream& log, Mmfile& mmf, Mmfile::iterator& iter, uint32_t& cksum,
	BitVector& deletes) {
	...
	case MM_COMMIT: {
		...
		log << "commit "
			...
			<< "deletes " << commit->ndeletes << ", " << ctime(&commit->t);
		/*{
		Mmoffset32* p;
		Mmoffset32* end;
		...
		cksum = checksum(
			cksum, (char*) commit + sizeof(int), iter.size() - sizeof(int));
		if (commit->cksum != cksum)
			log << "	ERROR commit had " << commit->cksum << " recover got "
				<< cksum << endl;
		cksum = checksum(0, 0, 0);
		break;
	}
