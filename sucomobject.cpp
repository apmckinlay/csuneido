...
#define A2OLE(s) \
	(((_lpa = (s)) == NULL) ? NULL \
							: (_convert = (strlen(_lpa) + 1), \
								  AtlA2WHelper((LPWSTR) _alloca(_convert * 2), \
									  _lpa, _convert)))
...
static void check_result(
	HRESULT hr, const char* progid, const char* name, const char* action) {
	if (hr == DISP_E_BADPARAMCOUNT)
		except("COM: " << progid << " " << action << " " << name
					   << " bad param count");
			...
			"COM: " << progid << " " << action << " " << name << " exception");
			...
			"COM: " << progid << " " << action << " " << name << " overflow");
...
static Value com2su(VARIANT* var) {
	...
	case VT_DISPATCH: {
		...
		if (idisp) {
			idisp->AddRef(); // VariantClear will release the reference from
							 // varValue
	...
	case VT_UNKNOWN: {
		...
		if (SUCCEEDED(hr) && idisp)
			...
			iunk->AddRef(); // VariantClear() will release the reference from
							// varValue
	...
	case VT_EMPTY:
		result = 0;
		break;
	case VT_BSTR: { // VariantClear() will release the BSTR in varValue
		int nw = SysStringLen(V_BSTR(&varValue));
		if (nw == 0)
			return SuEmptyString;
		int n = WideCharToMultiByte(
			CP_ACP, 0, V_BSTR(&varValue), nw, 0, 0, NULL, NULL);
		char* s = (char*) _alloca(n);
		n = WideCharToMultiByte(
			CP_ACP, 0, V_BSTR(&varValue), nw, s, n, NULL, NULL);
		if (n == 0)
	...
	case VT_DATE: {
		...
		DateTime dat = DateTime(sdt.wYear, sdt.wMonth, sdt.wDay, sdt.wHour,
			sdt.wMinute, sdt.wSecond, sdt.wMilliseconds);
...
Value SuCOMobject::getdata(Value member) {
	...
	hr = idisp->Invoke(dispid, IID_NULL, LOCALE_SYSTEM_DEFAULT,
		DISPATCH_PROPERTYGET, &args, &result, NULL, NULL);
...
void SuCOMobject::putdata(Value member, Value val) {
	...
	VARIANTARG arg;
	su2com(val, &arg);
	// Property puts have named arg that represents the value being assigned to
	// the property.
...
Value SuCOMobject::call(Value self, Value member, short nargs, short nargnames,
...
BUILTIN(COMobject, "(progid)") {
	...
	if (ARG(0).int_if_num(&n) && nullptr != (iunk = (IUnknown*) n)) {
		...
		if (SUCCEEDED(hr) && idisp)
			return new SuCOMobject(idisp, progid);
		hr = CoCreateInstance(
			clsid, NULL, CLSCTX_SERVER, IID_IUnknown, (void**) &iunk);
