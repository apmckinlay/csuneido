...
Index::Index(Database* d, TblNum t, const char* idx, Mmoffset r, short tl,
	int nn, bool k, bool u)
	: db(d), bt(d->dest, r, tl, nn), iskey(k), unique(u), tblnum(t),
	  idxname(idx) {
}
...
Index::iterator::iterator(Index* i, int tr, Key f, Key t, TranRead* trd)
	: ix(i), prevsize(_I64_MAX), tran(tr), from(f), to(t), rewound(true),
	  tranread(trd) {
}
...
void Index::iterator::operator++() {
	...
	while (!iter.eof() && (iter->adr() >= prevsize || !visible()))
		++iter;
	...
	if (tranread) {
		if (iter.eof()) {
			...
			if ((trace_level & TRACE_ALLINDEX) && ix->bt.get_nnodes() > 1 &&
				...
				TRACE(ALLINDEX,
					ix->db->get_table(ix->tblnum)->name << " " << ix->idxname);
...
TEST(index_single) {
	...
	Record recs[] = {record("andrew"), record("fred"), record("fred"),
		record("leeann"), record("leeann"), record("tracy")};
	...
	for (iter = index->begin(t, key("f"), key("m")), i = 1; i <= 4; ++iter, ++i)
		verify(!iter.eof() && iter->key.hasprefix(key1(recs[i])));
	for (iter = index->begin(t, key("fred"), key("leeann")), i = 1; i <= 4;
		 ++iter, ++i)
		verify(!iter.eof() && iter->key.hasprefix(key1(recs[i])));
	...
	for (iter = index->iter(t), --iter, i = n - 1; i >= 0; --iter, --i)
		verify(!iter.eof() && iter->key.hasprefix(key1(recs[i])));
	for (iter = index->iter(t, key("f"), key("m")), --iter, i = 4; i >= 1;
		 --iter, --i)
		verify(!iter.eof() && iter->key.hasprefix(key1(recs[i])));
	for (iter = index->iter(t, key("fred"), key("leeann")), --iter, i = 4;
		 i >= 1; --iter, --i)
		verify(!iter.eof() && iter->key.hasprefix(key1(recs[i])));
...
TEST(index_multi) {
	...
	for (iter = index->begin(
			 t, key("calgary", "leeann"), key("saskatoon", "fred")),
		i = 1;
		 i <= 4; ++iter, ++i)
		verify(!iter.eof() && iter->key.hasprefix(key2(recs[i])));
	...
	for (iter = index->iter(t), --iter, i = n - 1; i >= 0; --iter, --i)
		verify(!iter.eof() && iter->key.hasprefix(key2(recs[i])));
	for (iter = index->iter(t, key("calgary")), --iter, i = 2; i >= 0;
		 --iter, --i)
		verify(!iter.eof() && iter->key.hasprefix(key2(recs[i])));
	for (iter = index->iter(t, key("saskatoon")), --iter, i = 5; i >= 3;
		 --iter, --i)
		verify(!iter.eof() && iter->key.hasprefix(key2(recs[i])));
	for (iter =
			 index->iter(t, key("calgary", "leeann"), key("saskatoon", "fred")),
		--iter, i = 4;
		 i >= 1; --iter, --i)
		verify(!iter.eof() && iter->key.hasprefix(key2(recs[i])));
