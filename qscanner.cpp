...
int QueryScanner::next() {
	...
	do
		token = nextall();
	while (token == T_WHITE || token == T_NEWLINE || token == T_COMMENT);
	return token;
}
...
static Keyword words[] = {{"alter", K_ALTER}, {"and", T_AND},
	{"average", K_AVERAGE}, {"by", K_BY}, {"cascade", K_CASCADE},
	{"count", K_COUNT}, {"create", K_CREATE}, {"delete", K_DELETE},
	{"destroy", K_DROP}, {"drop", K_DROP}, {"ensure", K_ENSURE},
	{"extend", K_EXTEND}, {"false", K_FALSE}, {"history", K_HISTORY},
	{"in", K_IN}, {"index", K_INDEX}, {"insert", K_INSERT},
	{"intersect", K_INTERSECT}, {"into", K_INTO}, {"is", I_IS},
	{"isnt", I_ISNT}, {"join", K_JOIN}, {"key", K_KEY},
	{"leftjoin", K_LEFTJOIN}, {"list", K_LIST}, {"max", K_MAX}, {"min", K_MIN},
	{"minus", K_MINUS}, {"not", I_NOT}, {"or", T_OR}, {"project", K_PROJECT},
	{"remove", K_REMOVE}, {"rename", K_RENAME}, {"reverse", K_REVERSE},
	{"set", K_SET}, {"sort", K_SORT}, {"summarize", K_SUMMARIZE},
	{"sview", K_SVIEW}, {"times", K_TIMES}, {"to", K_TO}, {"total", K_TOTAL},
...
int QueryScanner::keywords(const char* s) {
	std::pair<Keyword*, Keyword*> p =
		std::equal_range(words, words + nwords, s, QscannerLt());
...
static int results[] = {K_ALTER, K_AVERAGE, K_BY, K_COUNT, K_CREATE, K_DELETE,
	K_DROP, K_DROP, K_ENSURE, K_EXTEND, K_FALSE, K_HISTORY, K_IN, K_INTO,
	K_INDEX, K_INSERT, K_INTERSECT, K_JOIN, K_KEY, K_LEFTJOIN, K_LIST, K_MAX,
	K_MIN, K_MINUS, K_PROJECT, K_REMOVE, K_RENAME, K_REVERSE, K_SET, K_SORT,
	K_SUMMARIZE, K_TIMES, K_TO, K_TOTAL, K_TRUE, K_UNION, K_UNIQUE, K_UPDATE,
	K_VIEW, K_WHERE};
