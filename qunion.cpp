...
Indexes Union::indexes() {
	...
	return intersect(intersect(source->keys(), source->indexes()),
...
double Union::optimize2(const Fields& index, const Fields& needs,
	...
	if (!nil(index)) {
		...
		// merge?
		Indexes keyidxs =
			intersect(intersect(source->keys(), source->indexes()),
				intersect(source2->keys(), source2->indexes()));
		Fields merge_key;
		...
		for (k = keyidxs; !nil(k); ++k) {
			...
			double cost =
				source->optimize1(*k, needs1, Fields(), is_cursor, false) +
		...
		for (k = source2->keys(); !nil(k); ++k) {
			...
			double cost =
				2 * source->optimize(none, needs1, needs1_k, is_cursor, false) +
		...
		for (k = source->keys(); !nil(k); ++k) {
			...
			double cost = 2 *
					source2->optimize(
						none, needs2, needs2_k, is_cursor, false) +
				source->optimize(*k, needs1, Fields(), is_cursor, false) +
				OUT_OF_ORDER;
			if (cost < cost2) {
				ki1 = *k;
				cost2 = cost;
...
Lisp<Fixed> Union::fixed() const {
	...
	for (f1 = fixed1; !nil(f1); ++f1)
		if (!member(cols2, f1->field))
			fix.push(
				Fixed(f1->field, set_union(f1->values, lisp(SuEmptyString))));
	Fields cols1 = source->columns();
	for (f2 = fixed2; !nil(f2); ++f2)
		if (!member(cols1, f2->field))
			fix.push(
				Fixed(f2->field, set_union(f2->values, lisp(SuEmptyString))));
	return fix;
}
	...
	if (strategy == LOOKUP) {
		...
		if (rewound) {
			...
			if (src1 || before(dir, key1, 1, curkey, 2)) {
				...
				row1 = source->get(dir);
				key1 = (row1 == Eof ? (dir == NEXT ? keymax : keymin)
			...
			if (src2 || before(dir, key2, 2, curkey, 1)) {
				...
				key2 = (row2 == Eof ? (dir == NEXT ? keymax : keymin)
