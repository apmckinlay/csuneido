...
const Transaction* Database::find_tran(int tran) {
	if (Transaction* t = get_tran(tran))
		return t;
	auto it = find_if(final.begin(), final.end(), FindTran(tran));
	if (it != final.end())
		return &*it;
	return NULL;
}
...
void Database::commit_update_tran(int tran) {
	...
	for (auto act = t->acts.begin(); act != t->acts.end(); ++act) {
		switch (act->type) {
		case CREATE_ACT: {
...
void Database::write_commit_record(
	int tran, const deque<TranAct>& acts, int ncreates, int ndeletes) {
	...
	if (ndeletes || ncreates || cksum != ::checksum(0, 0, 0)) {
		...
		Commit* commit =
			new (adr(alloc(n, MM_COMMIT))) Commit(tran, ncreates, ndeletes);
		...
		checksum(
			(char*) commit + sizeof(int), mmf->length(commit) - sizeof(int));
		commit->cksum = cksum;
...
bool Database::validate_reads(Transaction* t) {
	...
	auto begin = lower_bound(final.begin(), final.end(), t->asof);
	sort(t->reads.begin(), t->reads.end());
	TblNum cur_tblnum = -1;
	const char* cur_index = 0;
	short* colnums = 0;
	int nidxcols = 0;
	for (auto tr = t->reads.begin(); tr != t->reads.end(); ++tr) {
		if (tr->tblnum != cur_tblnum || tr->index != cur_index) {
			...
			nidxcols = 1 +
				count((const char*) tr->index,
					(const char*) strchr(tr->index, 0), ',');
		}
		// crude removal of record address from org & end
		Record from = tr->org;
		if (from.size() > nidxcols) {
			from = from.dup();
			from.truncate(nidxcols);
		}
		Record to = tr->end;
		if (to.size() > nidxcols) {
			to = to.dup();
			to.truncate(nidxcols);
		}

		for (auto iter = begin; iter != final.end(); ++iter) {
			for (auto act = iter->acts.begin(); act != iter->acts.end();
				 ++act) {
				...
				if (from <= key && key <= to) {
					t->conflict = read_conflict(&*iter, tr->tblnum, from, to,
						cur_index, key, act->type);
					return false;
				}
			}
		}
...
bool Database::finalize() {
	bool ok = true;
	auto oldest_t = min_element(trans.begin(), trans.end(), lt_asof);
	TranTime oldest =
		(oldest_t == trans.end() ? FUTURE : oldest_t->second.asof);
...
void Database::shutdown() {
	...
	for (auto i = trans.begin(); i != trans.end();) {
		int tran = i->first;
...
TEST(transaction_visibility) {
	...
	verify(
		r.getstr(0) == "fred"); // still old value, operating as-of start time
...
TEST(transaction_reads) {
	...
	int t1 = thedb->transaction(READONLY);
	thedb->get_index("test", "name")->begin(t1);
	verify(thedb->trans[t1].reads.empty());
	thedb->commit(t1);

	int t2 = thedb->transaction(READWRITE);
	thedb->get_index("test", "name")->begin(t2, key("a"), key("z"));
	Transaction& tran2 = thedb->trans[t2];
	verify(tran2.reads.size() == 1);
	TranRead& trd2 = tran2.reads.back();
	assert_eq(trd2.org, key("a"));
	verify(trd2.end.hasprefix(key("fred")));
	thedb->commit(t2);

	int t3 = thedb->transaction(READWRITE);
	auto iter = thedb->get_index("test", "name")->iter(t3, key("a"), key("z"));
	(void) --iter;
	Transaction& tran3 = thedb->trans[t3];
	verify(tran3.reads.size() == 1);
	TranRead& trd3 = tran3.reads.back();
	assert_eq(trd3.end, key("z"));
	verify(trd3.org.hasprefix(key("fred")));
	thedb->commit(t3);

	END
}

TEST(transaction_conflicts1) {
...
TEST(transaction_conflicts2) {
	// conflicting inserts (or updates)
	SETUP int t1 = thedb->transaction(READWRITE);
...
TEST(transaction_conflicts3) {
	// read validation problem
	SETUP int t = thedb->transaction(READWRITE);
