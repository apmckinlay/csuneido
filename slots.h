...
public:
	...
	void insert(
		const iterator& position, iterator first, const iterator& last) {
		int n = last - first;
		int i = position.i;
...
private:
	short t[(mem - sizeof(short) - sizeof(short)) /
		sizeof(short)]; // array of offsets
...
public:
	...
	VFslot back() {
		verify(sz > 0);
		return VFslot((char*) t + t[sz - 1].offset, t[sz - 1].adr.unpack());
	}
...
private:
	slot t[(mem - sizeof(short) - sizeof(short)) /
		sizeof(slot)]; // array of offsets
	short prev;        // points to start of heap (grows downward)
	short sz;
	int heapsize() {
		return sizeof t - prev;
