...
typedef Btree<FilterLeafSlot, FilterTreeSlot, FilterLeafSlots, FilterTreeSlots,
	TempDest>
	Filter;
...
struct Cmp {
	...
	Cmp(const gcstring& i, int o, const gcstring& v)
		: ident(i), op(o), value(v) {
	}
	Cmp(const gcstring& i, int o, const Lisp<gcstring>& v)
		: ident(i), op(o), values(v) {
	}
...
protected: // not private so tests can subclass and override
	...
	Fields source_index; // may have extra stuff on the end, or be missing
...
Select::Select(Query* s, Expr* e)
	: Query1(s), optFirst(true), getFirst(true), rewound(true), newrange(true),
	  conflicting(false), nrecs(-1), fltr(0), tran(-1), n_in(0), n_out(0),
	  fixdone(false) {
	e = e->fold();
	...
	if (!subset(source->columns(), expr->fields()))
		except("select: nonexistent columns: " << difference(
				   expr->fields(), source->columns()));
}
...
Query* Select::transform() {
	...
	// move select before extend, unless it depends on rules
		...
		if (!nil(src1))
			extend->source =
				new Select(extend->source, new And(src1.reverse()));
		if (!nil(rest))
	...
	// split select before & after summarize
		...
		if (!nil(src1))
			summarize->source =
				new Select(summarize->source, new And(src1.reverse()));
		if (!nil(rest))
			expr = new And(rest.reverse());
		else
			moved = true;
		...
		if ((*exprs)->term(fields)) {
			if (In* in = dynamic_cast<In*>(*exprs)) {
				...
				if (binop && binop->op != I_ISNT) {
					gcstring field =
						dynamic_cast<Identifier*>(binop->left)->ident;
					gcstring value =
						dynamic_cast<Constant*>(binop->right)->packed;
...
double Select::costwith(const Indexes& afilter, double primary_index_cost) {
	...
	double data_frac =
		includes(all, select_needs) && subset(primary, prior_needs)
		? 0
		: datafrac(all);
	...
	LOG(" = " << data_frac << " * " << tbl->totalsize() << " + "
			  ...
			  << data_frac * tbl->totalsize() + primary_index_cost +
			filter_cost);
...
void Select::select(const Fields& index, const Record& from, const Record& to) {
	...
	if (conflicting) {
		...
		LOG("converted to " << source_index << " " << sel.org << " => "
							<< sel.end);
	}
	rewound = true;
}
...
void Select::convert_select(
	...
	while (!nil(ri)) {
		if (!nil(si) && *si == *ri) {
			...
			except_err(this << endl
							<< " invalid select " << index << " " << from
							<< " to " << to);
	}
...
Row Select::get(Dir dir) {
	...
	if (rewound) {
		...
		newrange = true;
		range_i =
			(dir == NEXT ? -1 : ranges.size()); // allow for increment below
	}
	for (;;) {
		if (newrange) {
			Keyrange range;
...
static Keyrange intersect(const Keyrange& r1, const Keyrange& r2) {
	Keyrange result((r1.org < r2.org ? r2.org : r1.org),
...
bool Select::matches(Row& row) {
	...
	for (auto iter = isels.begin(); iter != isels.end(); ++iter) {
...
Ostream& operator<<(Ostream& os, const Iselect& r) {
	...
	return os << (r.org.d == 0 ? "[" : "]") << show(r.org.x) << ","
			  << show(r.end.x) << (r.end.d == 0 ? "]" : "[");
}
...
bool Iselect::inrange(const gcstring& x) {
	return (org.d == 0 ? (org.x <= x) : (org.x < x)) &&
