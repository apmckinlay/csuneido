...
const int DB_VERSION = 2; // increment for non-compatible format changes
...
Database::Database(const char* file, bool createmode)
	: tables(new Tables), clock(1), cksum(::checksum(0, nullptr, 0)),
	  output_type(MM_DATA) {
	...
	if (!mmf->first()) {
		...
		if (mmf->length(dbhdr()) < sizeof(Dbhdr) ||
...
void Database::open() {
	...
	Record r =
		find(schema_tran, indexes_index, key(TN_INDEXES, "table,columns"));
	verify(!nil(r) && r.off() == indexes);

	tables_index = mkindex(
		ckroot(find(schema_tran, indexes_index, key(TN_TABLES, "tablename"))));
	tblnum_index = mkindex(
		ckroot(find(schema_tran, indexes_index, key(TN_TABLES, "table"))));
	columns_index = mkindex(ckroot(
		find(schema_tran, indexes_index, key(TN_COLUMNS, "table,column"))));
	fkey_index = mkindex(ckroot(find(
		schema_tran, indexes_index, key(TN_INDEXES, "fktable,fkcolumns"))));
	...
	if (col != "-") { // addition of deleted field used by dump/load
		...
		for (Lisp<Col> cols = tbl->cols; !nil(cols); ++cols)
			if (cols->column == column)
				except("add column: column already exists: " << column << " in "
															 << table);
...
void Database::add_index(const gcstring& table, const gcstring& columns,
	...
	if (!colnums)
			...
			<< difference(commas_to_list(columns), get_columns(table)) << " in "
			<< table);
	for (Lisp<Idx> idxs = tbl->idxs; !nil(idxs); ++idxs)
		if (idxs->columns == columns)
			except("add index: index already exists: " << columns << " in "
													   << table);
	...
	if (!nil(tbl->idxs) && tbl->nrecords) {
		...
		for (auto iter = idx->begin(schema_tran); !iter.eof(); ++iter) {
			Record r(iter.data());
			if (fkey_source_block(
					schema_tran, fktbl, fkcolumns, project(r, colnums)))
				except("add index: blocked by foreign key: "
					<< columns << " in " << table);
			Record key = project(r, colnums, iter->adr());
			if (!index->insert(schema_tran, Vslot(key)))
				except("add index: duplicate key: " << columns << " = " << key
													<< " in " << table);
		}
	}
...
void Database::add_any_record(int tran, Tbl* tbl, Record& r) {
	...
	if (!loading)
		if (auto cols = fkey_source_block(tran, tbl, r))
			except("add record: blocked by foreign key: " << cols << " in "
														  << tbl->name);

	if (tbl->num > TN_VIEWS && r.size() > tbl->nextfield)
		except("output: record has more fields (" << r.size() << ") than "
												  << tbl->name << " should ("
												  << tbl->nextfield << ")");
...
void Database::add_index_entries(int tran, Tbl* tbl, Record r) {
	...
	for (Lisp<Idx> i = tbl->idxs; !nil(i); ++i) {
		...
		if (!i->index->insert(tran, Vslot(key))) {
			...
			except("duplicate key: " << i->columns << " = " << key << " in "
									 << tbl->name);
		}
	...
	for (Lisp<Idx> ix = tbl->idxs; !nil(ix); ++ix) {
		...
		for (Mmfile::iterator iter(first, mmf); iter != end; ++iter) {
			...
			if (!ix->index->insert(schema_tran, Vslot(key)))
				except("duplicate  " << ix->columns << " = " << key << " in "
									 << tbl->name);
			++n;
			if (off == last)
				break;
...
Mmoffset Database::update_record(
	...
	for (i = tbl->idxs; !nil(i); ++i) {
		...
		if (oldkey == newkey)
			continue;
		if ((block &&
				fkey_source_block(tran, get_table(i->fksrc.table),
					i->fksrc.columns, newkey)) ||
			fkey_target_block(tran, *i, oldkey, newkey))
			except(
				"update record in " << tbl->name << " blocked by foreign key");
	}

	if (!delete_act(tran, tbl->num, oldoff)) {
		Transaction* t = ck_get_tran(tran);
		except("update record in " << tbl->name
								   << " transaction conflict: " << t->conflict);
	}
	...
	for (i = tbl->idxs; !nil(i); ++i) {
		...
		if (!i->index->insert(tran, Vslot(newkey))) { // undo previous
			if (tran == schema_tran)
			...
			except("update record: duplicate key: "
				<< i->columns << " = " << newkey << " in " << tbl->name);
		}
		i->update();
	}
...
bool Database::is_system_table(const gcstring& table) {
	return table == "tables" || table == "columns" || table == "indexes" ||
		table == "views";
}
...
bool Database::is_system_column(const gcstring& table, const gcstring& column) {
	return //
		(table == "tables" &&
			(column == "table" || column == "nrows" ||
				column == "totalsize")) ||
		(table == "columns" &&
			(column == "table" || column == "column" || column == "field")) ||
		(table == "indexes" &&
			(column == "table" || column == "columns" || column == "root" ||
				column == "treelevels" || column == "nnodes"));
}
...
void Database::remove_column(const gcstring& table, const gcstring& column) {
	...
	for (Lisp<Idx> p = tbl->idxs; !nil(p); ++p) {
		...
		if (-1 != cols.find(col))
			except("delete column: can't delete column used in index: "
				<< column << " in " << table);
	}
	...
	if (nil(q))
			...
			"delete column: nonexistent column: " << column << " in " << table);
	tbl->cols.erase(*q);

	remove_any_record(
		schema_tran, "columns", "table,column", key(tbl->num, column));
}
...
void Database::remove_any_index(Tbl* tbl, const gcstring& columns) {
	...
	for (; !nil(p); ++p)
		if (p->columns == columns)
			break;
	if (nil(p))
		except("delete index: nonexistent index: " //
			<< columns << " in " << tbl->name);
	tbl->idxs.erase(*p);

	remove_any_record(
		schema_tran, "indexes", "table,columns", key(tbl->num, columns));
}
...
void Database::remove_record(int tran, Tbl* tbl, Record r) {
	if (tran != schema_tran) {
		...
		if (is_system_table(tbl->name))
			except("delete record: can't delete records from system table: "
				<< tbl->name);
	}
	verify(tbl);
	verify(!nil(r));
	...
	if (auto fktblname = fkey_target_block(tran, tbl, r))
		except("delete record from "
			<< tbl->name << " blocked by foreign key from " << fktblname);

	if (!delete_act(tran, tbl->num, r.off())) {
		Transaction* t = ck_get_tran(tran);
		except("delete record from "
			<< tbl->name << " transaction conflict: " << t->conflict);
	}
...
const char* Database::fkey_target_block(
	...
	for (Lisp<Fkey> fk = idx.fkdsts; !nil(fk); ++fk) {
		...
		if (!fkidx)
			continue;
		auto iter = fkidx->begin(tran, key);
...
// tables records
Record Database::record(TblNum tblnum, const gcstring& tblname, int nrows,
	int nextfield, int totalsize) {
	Record r;
	r.addval(tblnum);
	r.addval(tblname);
	r.addval(nextfield);
...
Record Database::record(TblNum tblnum, const gcstring& columns, Index* index,
	...
	index->getinfo(r);
	*r.alloc(24) =
		0; // 3 updatable int fields * max int packsize - min int packsize
	return r;
}
...
Index* Database::mkindex(Record r) {
	...
	return new Index(this, r.getint(I_TBLNUM), columns, r.getmmoffset(I_ROOT),
		r.getint(I_TREELEVELS), r.getint(I_NNODES), r.getval(I_KEY) == SuTrue,
		r.getval(I_KEY) == u);
}
...
bool Database::rename_table(const gcstring& oldname, const gcstring& newname) {
	...
	update_any_record(schema_tran, "tables", "table", key(tbl->num),
		record(
			tbl->num, newname, tbl->nrecords, tbl->nextfield, tbl->totalsize));

	tables->erase(
		oldname); // Note: table will be reloaded into tables on next use

	return true;
}

bool Database::rename_column(
	const gcstring& table, const gcstring& oldname, const gcstring& newname) {
	if (oldname == newname)
		return true;

	Tbl* tbl = ck_get_table(table);
	if (is_system_column(table, oldname))
		except("rename column: can't rename system column: "
			<< oldname << " in " << table);

	Col* col = NULL;
	for (Lisp<Col> cols = tbl->cols; !nil(cols); ++cols)
		if (cols->column == newname)
			except("rename column: column already exists: " << newname << " in "
															<< table);
		else if (cols->column == oldname)
			col = &(*cols);
	if (!col)
		except("rename column: nonexistent column: " << oldname << " in "
													 << table);
	...
	col->column = newname;
	remove_any_record(
		schema_tran, "columns", "table,column", key(tbl->num, oldname));
	...
	for (Lisp<Idx> idx = tbl->idxs; !nil(idx); ++idx) {
		...
		*i = newname;
		...
		remove_any_record(schema_tran, "indexes", "table,columns",
			key(tbl->num, idx->columns));
	}

	tables->erase(
		table); // Note: table will be reloaded into tables on next use
	return true;
}
...
TEST(database) {
	...
	verify(thedb->get_columns("test_database") ==
		lisp(gcstring("name"), gcstring("phone"), gcstring("num")));

	thedb->add_index("test_database", "name", true);
	thedb->add_index("test_database", "num", true);

	Record records[] = {record("bob", "123-4444", 1),
		record("joe", "652-9876", 2), record("axon", "249-5050", 3),
		record("andrew", "242-0707", 4), record("bobby", "123-4567", 5)};
	...
	xassert(thedb->update_record(
		tran, "test_database", "num", key(2), record("joe", "652-9876", 4)));
	thedb->update_record(
		tran, "test_database", "num", key(3), record("axon", "249-5051", 3));
	thedb->update_record(
		tran, "test_database", "num", key(3), record("axon", "249-5050", 3));
	verify(thedb->commit(tran));
	// verify
	tran = thedb->transaction(READONLY);
	iter = index->begin(tran);
