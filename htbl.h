 ...
 * Within blocks keys/values/distances are separate arrays to avoid padding.
...
public:
	...
	// move constructor
	Htbl_(Htbl_&& other)
		: blocks(other.blocks), isize(other.isize), readonly(other.readonly),
		  siz(other.siz) {
		other.reset();
	}
	...
	Htbl_& clear() {
		...
		siz = 0;
		if (blocks)
			memset(blocks, 0, nblocks() * sizeof(Block)); // help gc
		return *this;
	}
	...
	static constexpr const int primes[] = {16 - 5, // 11 + 4 (probelimit) = 15
		32 - 1,                                    // 31 + 5 (probelimit) = 36
		64 - 3, 128 - 1, 256 + 1, 512 - 3, 1024 - 3, 2048 + 5, 4096 - 3,
		8092 - 1, 16384 - 3, 32768 + 3, 65536 - 15};
	template <class T>
