// Copyright 2000 (c) Suneido Software Corp. All rights reserved.
...
#ifdef __MINGW32__
// const CLSID IID_IRichEditOle = { 0x00020D00, 0x00, 0x00, { 0xC0, 0x00, 0x00,
// 0x00, 0x00, 0x00, 0x00, 0x46 } };
#endif
const CLSID MyIID_IRichEditOleCallback = {
	0x00020D03, 0x00, 0x00, {0xC0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x46}};
...
STDMETHODIMP ITPCALL_QueryInterface(
	LPUNKNOWN punk, REFIID riid, LPUNKNOWN* ppvObj);
...
STDMETHODIMP ITPCALL_GetInPlaceContext(ITPCALL* pitpcall,
	LPOLEINPLACEFRAME FAR* ppipframe, LPOLEINPLACEUIWINDOW FAR* ppipuiDoc,
	LPOLEINPLACEFRAMEINFO pipfinfo);
STDMETHODIMP ITPCALL_ShowContainerUI(ITPCALL* pitpcall, BOOL fShow);
STDMETHODIMP ITPCALL_QueryInsertObject(
	ITPCALL* pitpcall, LPCLSID pclsid, LPSTORAGE pstg, LONG cp);
...
STDMETHODIMP ITPCALL_QueryAcceptData(ITPCALL* pitpcall, LPDATAOBJECT pdataobj,
	CLIPFORMAT* pcfFormat, DWORD reco, BOOL fReally, HGLOBAL hMetaPict);
STDMETHODIMP ITPCALL_ContextSensitiveHelp(ITPCALL* pitpcall, BOOL fEnterMode);
STDMETHODIMP ITPCALL_GetClipboardData(
	ITPCALL* pitpcall, CHARRANGE* pchrg, DWORD reco, LPDATAOBJECT* ppdataobj);
STDMETHODIMP ITPCALL_GetDragDropEffect(
	ITPCALL* pitpcall, BOOL fDrag, DWORD grfKeyState, LPDWORD pdwEffect);
STDMETHODIMP ITPCALL_GetContextMenu(ITPCALL* pitpcall, WORD seltype,
	LPOLEOBJECT poleobj, CHARRANGE* pchrg, HMENU* phmenu);

IRichEditOleCallbackVtbl ITPCALL_Vtbl = {(LPVOID) ITPCALL_QueryInterface,
	(LPVOID) ITPCALL_AddRef, (LPVOID) ITPCALL_Release,
	(LPVOID) ITPCALL_GetNewStorage, (LPVOID) ITPCALL_GetInPlaceContext,
	(LPVOID) ITPCALL_ShowContainerUI, (LPVOID) ITPCALL_QueryInsertObject,
	(LPVOID) ITPCALL_DeleteObject, (LPVOID) ITPCALL_QueryAcceptData,
	(LPVOID) ITPCALL_ContextSensitiveHelp, (LPVOID) ITPCALL_GetClipboardData,
	(LPVOID) ITPCALL_GetDragDropEffect, (LPVOID) ITPCALL_GetContextMenu};
...
STDMETHODIMP ITPCALL_GetNewStorage(ITPCALL* pitpcall, LPSTORAGE FAR* ppstg) {
	...
	if (NOERROR !=
		pitpcall->pstg->lpVtbl->CreateStorage(pitpcall->pstg, szItemW,
			STGM_READWRITE | STGM_TRANSACTED | STGM_SHARE_EXCLUSIVE, 0, 0,
			ppstg))
		return E_FAIL;
	return S_OK;
}
...
void RichEditOle(HWND hwndRE) {
	...
	if (NOERROR !=
		StgCreateDocfile(NULL,
			STGM_SHARE_EXCLUSIVE | STGM_READWRITE | STGM_TRANSACTED |
