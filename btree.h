...
template <class LeafSlot, class TreeSlot, class LeafSlots, class TreeSlots,
	class Dest>
class Btree {
	...
	friend void test_btree_rangefrac_onelevel();
	friend void test_btree_rangefrac_multilevel();
...
private:
	...
	struct LeafNode {
		...
		Insert insert(const LeafSlot& x) { // returns false if no room
			LeafSlotsIterator slot =
				std::lower_bound(slots.begin(), slots.end(), x);
		...
		bool erase(const Key& key) {
			LeafSlotsIterator slot =
				std::lower_bound(slots.begin(), slots.end(), LeafSlot(key));
	...
	struct TreeNode {
		...
		bool insert(const Key& key, Mmoffset off) { // returns false if no room
			TreeSlotsIterator slot =
				std::lower_bound(slots.begin(), slots.end(), TreeSlot(key));
	...
	public:
		...
		Mmoffset adr(int pos) const override {
			return pos == node->slots.size() ? node->lastoff.unpack()
											 : node->slots[pos].adr;
		}
...
public:
	...
	public:
		...
		void operator++() {
			...
			LeafSlotsIterator t =
				std::upper_bound(leaf->slots.begin(), leaf->slots.end(), cur);
			if (t < leaf->slots.end())
				cur.copy(*t);
		...
		void operator--() {
			...
			LeafNode* leaf = (LeafNode*) bt->dest->adr(off);
			LeafSlotsIterator t =
				std::lower_bound(leaf->slots.begin(), leaf->slots.end(), cur);
		...
		bool seek(const Key& key) {
			...
			LeafSlotsIterator t = std::lower_bound(
				leaf->slots.begin(), leaf->slots.end(), LeafSlot(key));
			valid = bt->modified;
			bool found;
			if (t == leaf->slots.end()) {
				--t;
	...
	//---------------------------------------------------------------
	explicit Btree(Dest* d)
		: root_(0), treelevels(0), nnodes(0), modified(0), dest(d) {
	}
	...
	bool insert(LeafSlot x) { // returns false for duplicate key
		...
		++nnodes;
		verify(OK ==
			(x <= left->slots.back() ? left->insert(x) : leaf->insert(x)));
		...
		for (--i; i >= 0; --i) {
			...
			++nnodes;
			verify(tleft->slots.back().key < key ? nodes[i]->insert(key, off)
												 : tleft->insert(key, off));
	...
	float rangefrac(const Key& from, const Key& to) {
		...
		// con() << "fromPos " << fromPos <<
		// 	" toPos " << toPos << " = " << max(toPos - fromPos, MIN_FRAC) <<
		// endl;
	...
	float* getChildFracs(Node* node) {
		...
		for (int i = 0; i < n; ++i) {
			total += childSizes[i] =
				node->level() >= treelevels ? 1 : childNode(node, i)->size();
			// con() << " " << childSizes[i];
		}
		// con() << endl;
