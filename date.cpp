...
static bool DateCalc_leap_year(int year) {
	...
	return (((year & 0x03) == 0) &&
		((((yy = (int) (year / 100)) * 100) != year) || ((yy & 0x03) == 0)));
}
...
static int DateCalc_Date_to_Days(int year, int month, int day) {
	...
	if ((year >= 1) && (month >= 1) && (month <= 12) && (day >= 1) &&
		(day <=
			DateCalc_Days_in_Month_[leap = DateCalc_leap_year(year)][month]))
...
bool DateTime::valid() const {
	...
	return DateCalc_check_date(year, month, day) &&
		DateCalc_check_time(hour, minute, second) && 0 <= millisecond &&
		millisecond <= 999;
}

static bool DateCalc_delta_hms(int* Dd, int* Dh, int* Dm, int* Ds, int hour1,
	int min1, int sec1, int hour2, int min2, int sec2) {
	int HH;
	int MM;
	int SS;
...
int64_t DateTime::minus_milliseconds(DateTime& dt2) {
	...
	DateCalc_delta_hms(&dd, &dh, &dm, &ds, dt2.hour, dt2.minute, dt2.second,
		hour, minute, second);
	int dms = millisecond - dt2.millisecond;
	return (((((int64_t) minus_days(dt2) + dd) * 24 + dh) * 60 + dm) * 60 +
			   ds) *
		1000 +
		dms;
}
...
static bool DateCalc_add_delta_dhms(int* year, int* month, int* day, int* hour,
	int* min, int* sec, int Dd, int Dh, int Dm, int Ds) {
	if (DateCalc_check_date(*year, *month, *day) &&
		DateCalc_check_time(*hour, *min, *sec)) {
		DateCalc_Normalize_Ranges(&Dd, &Dh, &Dm, &Ds);
		Ds +=
			((((*hour * 60L) + *min) * 60L) + *sec) + (((Dh * 60L) + Dm) * 60L);
		while (Ds < 0L) {
			Ds += 86400L;
			Dd--;
		}
...
static bool DateCalc_add_delta_ymdhms(int* year, int* month, int* day,
	int* hour, int* min, int* sec, int D_y, int D_m, int D_d, int Dh, int Dm,
	int Ds) {
	if (!(DateCalc_check_date(*year, *month, *day) &&
			DateCalc_check_time(*hour, *min, *sec)))
		return false;
	...
	D_d += *day - 1;
	*day = 1;
	return DateCalc_add_delta_dhms(
		year, month, day, hour, min, sec, D_d, Dh, Dm, Ds);
}
