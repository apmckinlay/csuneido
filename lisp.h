...
public:
	...
	friend T& car(const Lisp& list)
		{ verify(list.first); return list.first->value; }
	T& operator*() const
		{
		verify(first);
		return first->value;
	...
	friend Lisp cdr(const Lisp& list)
		{ verify(list.first); return Lisp<T>(list.first->next); }
	Lisp& operator++()
		{ if (first) first = first->next; return *this; }
	Lisp operator++(int)
		{
		Lisp ret = *this;
		if (first) first = first->next;
...
template <class T> struct EqFirst
	{
...
template <class T> Lisp<T> reverse(const Lisp<T>& list)
	{
	Lisp<T> rev;
...
template <class T> bool member(Lisp<T> list, const T& t)
	{
...
template <class T1, class T2> int search(Lisp<T1> list, const T2& t)
	{
	int i;
...
template <class T1, class T2> Lisp<T1> find(Lisp<T1> list, const T2& t)
	{
...
template <class T> Lisp<T> erase(Lisp<T> list, const T& t)
	{
	Lisp<T> result;
...
template <class T> bool prefix(Lisp<T> x, Lisp<T> y)
	{
...
template <class T> Lisp<T> concat(Lisp<T> x, Lisp<T> y)
	{
	Lisp<T> list;
	for (; ! nil(x); x = cdr(x))
...
template <class T> Lisp<T> intersect(Lisp<T> x, const Lisp<T>& y)
	{
	Lisp<T> list;
	for (; ! nil(x); x = cdr(x))
		{
...
template <class T> Lisp<T> difference(Lisp<T> x, const Lisp<T>& y)
	{
	Lisp<T> list;
	for (; ! nil(x); x = cdr(x))
		{
...
template <class T> Lisp<T> lispset(Lisp<T> x)
	{
	Lisp<T> list;
	for (; ! nil(x); x = cdr(x))
		{
...
template <class T> bool subset(const Lisp<T>& x, Lisp<T> y)
	{
	for (; ! nil(y); y = cdr(y))
		if (! member(x, car(y)))
			return false;
...
template <class T> Lisp<T> set_union(const Lisp<T>& x, Lisp<T> y)
	{
...
template <class T> bool set_eq(const Lisp<T>& x, const Lisp<T>& y)
	{
	int xn = size(x);
...
template <class T> bool prefix_set(Lisp<T> list, const Lisp<T>& set)
	{
	int n = size(set);
	int i = 0;
...
template <class T> Ostream& operator<<(Ostream& os, Lisp<T> list)
	{
	os << '(';
	for (; ! nil(list); ++list)
		{
...
// used by sort
template <class T> void split(Lisp<T> z, Lisp<T>& x, Lisp<T>& y)
	{
	while (! nil(z))
		{
		x.push(*z++);
...
// used by sort
template <class T> Lisp<T> merge(Lisp<T> x, Lisp<T> y)
	{
	if (nil(x))
		return y;
	if (nil(y))
...
// merge sort a list
template <class T> Lisp<T> sort(const Lisp<T>& x)
	{
	if (nil(x) || nil(cdr(x)))
		return x;
	Lisp<T> part1, part2;
