...
struct AutoTran {
	AutoTran(int t)
		: tin(t), tran(t <= 0 ? theDB()->transaction(READONLY) : t) {
	}
	~AutoTran() {
		if (tin <= 0)
			theDB()->commit(tran);
...
Row DbmsLocal::get(
	Dir dir, const char* querystr, bool one, Header& hdr, int t) {
	AutoTran tran(t);
...
int update_request(
	int tran, Query* q, const Lisp<gcstring>& c, const Lisp<Expr*>& exprs) {
	Lisp<Value> cols;
