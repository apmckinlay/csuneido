...
double Rename::optimize2(const Fields& index, const Fields& needs,
	const Fields& firstneeds, bool is_cursor, bool freeze) {
	// NOTE: optimize1 to bypass tempindex
	return source->optimize1(rename_fields(index, to, from),
		rename_fields(needs, to, from), rename_fields(firstneeds, to, from),
		is_cursor, freeze);
}
