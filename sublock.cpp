...
void SuBlock::persist()
	{
	if (within(proc->stack, frame->local))
		{
		...
		verify(within(proc->frames, frame));
		for (Frame* f = proc->fp; f > proc->frames && 
			(! within(proc->stack, f->local) || f->local >= old_locals); 
			--f)
			{
